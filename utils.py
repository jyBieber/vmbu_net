...
class FocalTverskyLoss(nn.Module):
    ...
    输入 preds 可为 logits 或概率（0~1）。函数内部会自动对 logits 做 sigmoid 判断。
    """
    ...
    def forward(self, preds, targets):
        # preds: (N,1,H,W) logits or probs
        # targets: (N,1,H,W) or (N,H,W)
        if preds.dim() == 4 and targets.dim() == 3:
            targets = targets.unsqueeze(1)  # (N,H,W) -> (N,1,H,W)

        # 如果 logits 范围超出 [0,1]，则视为 logits，做 sigmoid
...
class FTLBDLoss(nn.Module):
    def __init__(self, base_loss=BceDiceLoss(), alpha=1.0, beta=0.5):
        super().__init__()
        self.base_loss = base_loss
        # 如果未提供ftl，创建默认的FocalTverskyLoss
        self.ftl = FocalTverskyLoss(alpha=0.7, beta=0.3, gamma=0.75, smooth=1e-6)
        # 使用正确的参数名
        self.alpha = alpha  # 基础损失权重
        self.beta = beta  # FTL损失权重
    ...
    def forward(self, pred, target):
        ...
        # FocalTverskyLoss - 只传入pred和target
        penalty = self.ftl(pred, target)

        # 组合损失
        return self.alpha * base_loss + self.beta * penalty

class DistanceMapLoss(nn.Module):
    def __init__(self, base_loss=BceDiceLoss(),alpha=1.0, beta=0.5):
        super().__init__()
        self.base_loss = base_loss  # 基础损失（如交叉熵）
        self.alpha = alpha  # 基础损失权重
        self.beta = beta  # 距离图惩罚项权重
    ...
    def forward(self, pred, target, distance_map):
        ...
        penalty = torch.mean(distance_map * (pred_prob - target) ** 2)
