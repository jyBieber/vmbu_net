...
class BoundaryCrossAttention(CrossAttention):
    ...
    def forward(self, tgt, src):
        ...
        # === ADD/CHANGE ===
        # Save last weights to module instance for external visualization (test only)
        # detach to cpu to avoid holding computation graph / GPU tensors
        try:
            self.last_weights = weights.detach().cpu()  # shape typically (B,1,H,W) or (B,C,H,W)
        except Exception:
            # in case weights is not a tensor or something unexpected, set None
